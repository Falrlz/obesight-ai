...
**Obesight AI** helps you monitor, assess, and understand your obesity risk level and provides personalized lifestyle recommendations using a high-performance Machine Learning (ML) model (Random Forest (RF)) and an interactive bilingual dashboard.
...
- Input features are normalized and processed by a pre-trained **RF Classifier** pipeline to identify weight risk categories.
...
- Renders an interactive results dashboard featuring a Body Mass Index (BMI) gauge, AI probability distribution charts, and a checkable action plan.
...
- **Dynamic Analytics Dashboard**: Renders interactive SVG components, including a custom BMI Gauge aligning with World Health Organization (WHO) ranges, and horizontal confidence bar charts built with **Recharts** displaying model class probabilities.
...
The backend is a high-performance, stateless Representational State Transfer (REST) Application Programming Interface (API) built using **FastAPI** and **Python 3.11+**, acting as the bridge between user inputs and the ML model.
...
- **Model Comparison & Evaluation**: Baseline training runs compare 5 core estimators: K-Nearest Neighbors (KNN), Support Vector Machine (SVM), RF, Extreme Gradient Boosting (XGBoost), and Light Gradient Boosting Machine (LightGBM) on stratified validation splits.
...
Multiple ML model pipelines were trained and evaluated on stratified validation splits. Below are the performance metrics compiled from training runs:

| Model Candidate | Accuracy | F1-Score | Precision | Recall |
| :--- | :---: | :---: | :---: | :---: |
| **Tuned RF (Best)** | **99.28%** | **99.28%** | **99.30%** | **99.28%** |
| Baseline RF | 99.04% | 99.04% | 99.06% | 99.04% |
...
The winning model (**Tuned RF**) achieved **99.28% validation accuracy** and is exported as the production inference pipeline (`best_model.pkl`).
...
### Machine Learning
- **Scikit-Learn** → ML pipelines, scaling, and classification algorithms (KNN, SVM, RF)
- **LightGBM** & **XGBoost** → Gradient boosting frameworks compared in baseline training
